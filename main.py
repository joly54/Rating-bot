...
def update(students):
    try:
        ...
        formating = []
        colors = generate_color_array((count + 1) - int(count * 0.15))
        fmt = cellFormat(
            backgroundColor=color(1, 1, 1),
        )
        formating.append(
            (f"{get_column_letter(1)}{1}:{get_column_letter(500)}{500}", fmt)
        )
        for i in range(1 + int(count * 0.15), count + 1):
            fmt = cellFormat(
                backgroundColor=color(
                    colors[i - int(count * 0.15)][0],
                    colors[i - int(count * 0.15)][1],
                    colors[i - int(count * 0.15)][2],
                ),
                textFormat=textFormat(bold=False),
                horizontalAlignment="CENTER",
            )
            formating.append(
                (f"{get_column_letter(1)}{i+1}:{get_column_letter(5)}{i+1}", fmt)
            )
        for i in range(1, +int(count * 0.15) + 1):
            fmt = cellFormat(
        ...
        if len(students) > count:
            for i in range(count, len(students)):
                ...
                formating.append(
                    (f"{get_column_letter(1)}{i+2}:{get_column_letter(5)}{i+2}", fmt)
                )
    ...
    try:
            ...
            if message.text.startswith("/"):
                ...
                if command == "edit":
                    ...
                    try:
                        ...
                        print(e)
                        bot.send_message(800918003, str(e))
                elif user_id == "800918003" and command.find("exit") != -1:
                    try:
                        global is_run
                        is_run = 0
                        bot.send_message(user_id, "Is_run = 0")
                        sys.exit()
                    except Exception as e:
                        print(e)
                        bot.send_message(800918003, str(e))
                ...
                if text.isnumeric() and (int(text) == 1 or int(text) == 2):
                    ...
                    bot.send_message(
                        user_id,
                        "Вашу групу додано, тепер надішліть своє ім'я та прізвище",
                    )
        ...
        try:
            if user_id != "800918003":
                ...
                date_time = now.strftime("Time %H-%M-%S Date %d-%m-%y")
                print(
                    f"New message from\t\t{name}({user_id}):\t\t{text}\t{date_time}\t\tStatus: {students[user_id][3]}\n"
                )
